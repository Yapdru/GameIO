// Obby Run - parkour obstacle course
...
export class ObbyGame {
  ...
  generatePlatforms() {
    ...
    // Create a course with increasingly difficult platforms
    let y = this.height - 150;
    let x = 100;

    for (let i = 0; i < 15; i++) {
      const width = 80 - i * 3; // Platforms get narrower
      const gap = 40 + i * 3; // Gaps get bigger
  ...
  update() {
    ...
    this.platforms.forEach(platform => {
      ...
      if (
        ...
        this.player.jumpsLeft = 1; // Reset jumps on landing
      }
    });
    ...
    this.checkpoints.forEach((cp, i) => {
      if (i > this.currentCheckpoint) {
        ...
        if (dist < checkpointRadius) {
          ...
          this.score += Math.max(50 - lapTime * 10, 10); // Time bonus
          this.checkpointTime = Date.now();
        }
      }
    });
    ...
    if (this.currentCheckpoint === this.checkpoints.length - 1) {
      ...
      if (dist < 60 && !this.finished) {
        this.finished = true;
        this.score += 100; // Finish bonus
        this.stop();
      }
    }

    // Fall off map = end game
